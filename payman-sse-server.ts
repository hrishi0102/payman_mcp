    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to create US_ACH payee: ${
              error instanceof Error ? error.message : String(error)
            }`,
          },
        ],
    ...
    try {
      ...
      if (!response.ok) {
        return {
          content: [
            {
              type: "text",
              text: `Error creating CRYPTO_ADDRESS payee: ${JSON.stringify(
                data
              )}`,
            },
          ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `CRYPTO_ADDRESS payee created successfully: ${JSON.stringify(
              data
            )}`,
          },
        ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to create CRYPTO_ADDRESS payee: ${
              error instanceof Error ? error.message : String(error)
            }`,
          },
        ],
...
server.tool(
  "send-payment",
  {
    ...
    metadata: z
      .record(z.any())
      .optional()
      .describe("Optional metadata for the payment"),
  },
      ...
      if (!response.ok) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error searching payees (Status ${
                response.status
...
server.tool("get-balance", {}, async () => {
  ...
  try {
    const response = await fetch(
      "https://agent.payman.ai/api/balances/currencies/TSD",
      {
        method: "GET",
        headers: {
          "x-payman-api-secret": paymanApiKey,
    ...
    if (!response.ok) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error fetching balance (Status ${
              response.status
            }): ${JSON.stringify(errorData)}`,
          },
        ],

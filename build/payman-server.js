...
// Store API key (normally would be loaded from environment or config)
let paymanApiKey = "";
// Tool to set API key
server.tool("set-api-key", { apiKey: z.string().describe("The Payman API key to use for authentication") }, async ({ apiKey }) => {
    paymanApiKey = apiKey;
    return {
        content: [{
                type: "text",
...
server.tool("search-payees", {}, async () => {
    if (!paymanApiKey) {
        return {
            content: [{
                    type: "text",
    ...
    try {
        // Using the exact headers that work in Postman
        const response = await fetch('https://agent.payman.ai/api/payments/search-payees', {
            method: 'GET',
            headers: {
                'x-payman-api-secret': paymanApiKey,
                'content-type': 'application/json' // Adding this as shown in the working curl command
            }
        });
        // Handle the response
        if (!response.ok) {
